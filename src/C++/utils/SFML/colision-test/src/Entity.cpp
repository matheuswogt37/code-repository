...
void Entity::handleEntityInput(sf::Keyboard::Key k, bool pressed) {
    switch (k) {
        case sf::Keyboard::W:
            this->mUp = pressed;
            break;
...
void Entity::loadAnim() {
    // set values to animation
    this->anim.setValues(&this->texture, sf::Vector2u(8, 12), 0.5f);
}
...
bool Entity::getMDown() {
    return this->mDown;
}

bool Entity::getMLeft() {
    return this->mLeft;
}

bool Entity::getMRight() {
    return this->mRight;
}
...
void Entity::setSpritePos(float x, float y) {
...
void Entity::setSpriteMove(sf::Vector2f mov) {
    this->sprite.move(mov);
}

void Entity::setFaceRight(bool newFR) {
